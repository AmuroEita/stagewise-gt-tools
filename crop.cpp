...
std::pair<std::vector<float>, int> read_fvecs(const char* filename, int n) {
    ...
    char* fileptr = static_cast<char*>(mmap(0, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0));
    ...
    if (n > total_vectors) {
        std::cerr << "Requested " << n << " vectors, but file only has " << total_vectors << std::endl;
        n = total_vectors; 
    }
...
void write_fvecs(const char* filename, const std::vector<float>& data, int n, int dim) {
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cout << "Usage: " << argv[0] << " <input.fvecs> <num_vectors> <output.fvecs>" << std::endl;
        return 1;
    }
    ...
    std::cout << "Read " << n << " vectors with dimension " << dim << " from " << input_file << std::endl;
