...
find_k_nearest(const std::vector<uint8_t> &query,
               const std::vector<std::vector<uint8_t>> &base, size_t b_size,
               int k) {
...
compute_batch_groundtruth(const std::vector<std::vector<uint8_t>> &base,
...
std::vector<std::vector<uint8_t>> read_fvecs(const std::string &filename) {
  ...
  while (in.good()) {
    ...
    for (int i = 0; i < dim; ++i) {
      vec[i] =
          static_cast<uint8_t>(std::min(std::max(vec_float[i], 0.0f), 255.0f));
    }
    data.push_back(vec);
  }
  in.close();
