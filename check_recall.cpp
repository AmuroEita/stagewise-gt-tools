...
ThreadResult process_chunk(
    ...
    const std::unordered_map<
        size_t, std::unordered_map<size_t, std::unordered_set<uint32_t>>>& gt_map,
...
if (gt_entry.tags.size() != gt_entry.distances.size()) {
std::cerr << "Error: Tags and distances size mismatch for insert_offset "
       << gt_entry.insert_offset << ", query_idx " << gt_entry.query_idx << std::endl;
return 0.0f;
}

auto& tag_set = gt_map[gt_entry.insert_offset][gt_entry.query_idx];
tag_set.reserve(recall_at);
...
size_t tie_breaker = std::min(recall_at - 1, tagged_distances.size());
while (tie_breaker < tagged_distances.size() &&
tagged_distances[tie_breaker].second == tagged_distances[recall_at - 1].second) {
tie_breaker++;
}
for (size_t i = 0; i < tie_breaker; ++i) {
tag_set.insert(tagged_distances[i].first);
}
ties_detected += (tie_breaker > recall_at) ? (tie_breaker - recall_at) : 0;
}
...
float average_recall = total_recall / valid_entries * 100.0f; // 转换为百分比，与全局 recall 一致
...
float batch_avg_recall = recall_sum / sorted_batch_entry_count[offset] * 100.0f;
...
<< " queries, average stage-wise recall: " << average_recall
<< "%" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        float recall = check_recall(res, gt, recall_path, 10);
