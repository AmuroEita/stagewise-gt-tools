...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " gt_path result_path" << std::endl;
        return 1;
    }
...

    {
        ...
        if (qid != num_queries) {
            std::cerr << "Number of queries mismatch: gt " << num_queries << ", result " << qid << std::endl;
            return 1;
        }
    }
    uint32_t recall_at = dim_or;
    float* gs_dist = nullptr;
    double recall = calculate_recall(num_queries, gold_std_vec.data(), gs_dist, dim_gs, our_results_vec.data(), dim_or, recall_at);
