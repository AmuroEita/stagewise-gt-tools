...
float euclidean_distance_simd(const std::vector<uint8_t> &a,
    ...
    if (n >= 32) {
        ...
        for (; i <= n - 32; i += 32) {
            ...
            __m256i va_low =
                _mm256_cvtepu8_epi16(_mm256_extracti128_si256(va, 0));
            __m256i vb_low =
                _mm256_cvtepu8_epi16(_mm256_extracti128_si256(vb, 0));
            __m256i va_high =
                _mm256_cvtepu8_epi16(_mm256_extracti128_si256(va, 1));
            __m256i vb_high =
                _mm256_cvtepu8_epi16(_mm256_extracti128_si256(vb, 1));
            __m256 diff_low =
                _mm256_cvtepi32_ps(_mm256_sub_epi32(va_low, vb_low));
            __m256 diff_high =
                _mm256_cvtepi32_ps(_mm256_sub_epi32(va_high, vb_high));
...
std::vector<PointPair> find_k_nearest(
    const std::vector<uint8_t> &query,
    const std::vector<std::vector<uint8_t>> &base, size_t b_size, int k) {
...
std::vector<std::vector<PointPair>> compute_batch_groundtruth(
    ...
    const std::vector<std::vector<uint8_t>> &queries, size_t b_size, int k) {
...
int main(int argc, char *argv[]) {
    ...
    if (args.base_path.empty() || args.query_path.empty() ||
        args.gt_path.empty()) {
