...
bool concurrent_bench(const std::string &data_path,
  ...
  std::cout << "Starting concurrent benchmarking with #threads: " << num_threads
            << " #ratio: " << write_ratio << ":" << 1 - write_ratio
            << std::endl;
    ...
    for (size_t idx = start_insert_offset; idx < end_insert_offset; ++idx) {
      pool.enqueue_task([&, idx] {
        try {
          ...
          int insert_result =
              index->insert_point(&data[(idx + begin_num) * aligned_dim],
    ...
    for (size_t idx = start_search_offset; idx < end_search_offset; ++idx) {
      ...
      pool.enqueue_task([&, query_idx] {
        try {
          ...
          index->search_with_tags(query + query_idx * query_aligned_dim,
                                  recall_at, Ls, query_result_tags);
          auto qe = std::chrono::high_resolution_clock::now();
          std::chrono::duration<double> diff = qe - qs;
          {
          ...
          }
          {
            ...
            search_results.emplace_back(end_insert_offset * batch_size, query_idx,
                                        query_result_tags);
          }
        } catch (...) {
          std::unique_lock<std::mutex> lock(last_except_mutex);
  ...
  double mean_insert_latency =
      std::accumulate(insert_latency_stats.begin(), insert_latency_stats.end(),
                      0.0) /
  ...
  double mean_search_latency =
      std::accumulate(search_latency_stats.begin(), search_latency_stats.end(),
                      0.0) /
