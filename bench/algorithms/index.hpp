...
class IndexBase {
   public:
    ...
    virtual int batch_insert(const T* batch_data,
                             const TagT* batch_tags, size_t num_points) = 0;
    ...
    virtual int batch_search(const T* batch_queries, uint32_t k,
                              ...
                              TagT** batch_results) = 0;

    void get_insert_latencies(std::vector<double> &times) { times = insert_latencies; }

    void get_search_latencies(std::vector<double> &times) { times = search_latencies; }
};